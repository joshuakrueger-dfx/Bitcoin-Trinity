...
3. ~~**Kyoto peer selection on block download**~~ — ✅ **Resolved 2026-08-11.** Read directly against `github.com/rustaceanrob/kyoto` (published crate `bip157`, `Cargo.toml` confirms `name = "bip157"`, matches `0.6.3`). `src/node.rs::get_blocks` dispatches queued block requests via `src/network/peer_map.rs::send_random`, which picks uniformly at random (`StdRng::from_entropy()`) among all connected peers — **not** the peer that supplied the matching filter (that `peer_id` is used only for ban-on-validation-error, never for routing). So block-fetch goes to a **uniformly random peer**, not a deliberately *different* peer and not the *same* peer by design — better than the worst case, but no netgroup diversity or origin-peer exclusion either. Consequence in 1.6 and O3 below.
...
**Legend:** 🟥 sees key material · 🟨 trust boundary · ⬜ never key material. This legend is not the security-critical surface: a ⬜ crate can still lose money. The surface, its tiers, and the gates that are narrower than it are §1.8.
...
The crate `trinity-ffi` is the **only** one with `#[uniffi::export]`. It is full-tier on the security-critical surface (§1.8): this boundary decides what the compromise-able JS layer can touch. Everything that crosses it is listed here completely:
...
impl TrinityCore {
    ...
    /// Fails with SignError::SpendLimitExceeded when the policy demands a
    /// passphrase; the platform then calls `sign_ab_with_passphrase`.
...
This crate is full-tier on the security-critical surface (§1.8): `broadcast` is the path money leaves the device, and `full_scan`/`sync` feed the balance the user acts on.
...
2. **The CBF privacy claim, now evidenced (Section 0.3, gap 3, resolved 2026-08-11):** `bip157 0.6.3` routes block-fetch-after-filter-match to a **uniformly random connected peer**, not deliberately back to the filter-supplying peer — but also without netgroup diversity or origin-peer exclusion. The UI label must reflect exactly that: "more private than a third-party Electrum server" remains correct; "private" or "anonymous" unqualified would overstate a random-selection mechanism as a hardened anti-correlation design.
...
| Signature-path budget | Hard upper bound on the transitive external dependency count of a **named subset** of the security-critical surface (§1.8): `trinity-types`, `-entropy`, `-keystore`, `-signer`, and `-verify` (only `-e normal`, without dev and build deps). These five see key material or independently verify a PSBT. `trinity-chain`, `trinity-watch`, and `trinity-ffi` sit on that surface but are excluded here because they pull the Electrum/Core/CBF, BDK-wallet, and uniffi stacks — folding them in would make the ceiling stop constraining the secret-handling path. **Measured as the union over the shipped mobile targets `aarch64-apple-ios` and `aarch64-linux-android` (not the developer host): 52 external crates. Gate at 55.** The number comes from `scripts/dep_budget.py` (`MEASURED`), not from an estimate; raising only with justification in the PR. For comparison: `trinity-verify` alone gets by with **23**. |

> **Honest note on React Native:** The JS layer brings hundreds of npm dependencies. These never see a secret (1.3) and are outside the secret-handling subset of §1.8, but they can **display whatever they want** — in particular a wrong recipient address. The verifier (1.5) and the native confirmation display (Section 6.2) are the answer. The npm supply chain is thus not harmless, but reduced to "can deceive, cannot steal".

### 1.8 The security-critical surface

Four gates used to carry four different crate lists for the same idea — the surface where a defect costs money. This section is the single definition. Every other list is either this table or a named, justified subset of it.

**Security-critical** means: a defect here can spend funds, make funds unrecoverable, leak key material, or show a balance or send-state that causes the user to send again. The 🟥/⬜ legend in 1.1 answers a different question (who sees secrets). `trinity-chain` is ⬜ and still sits here.

| Crate | Tier | What goes wrong, and what it costs |
|---|---|---|
| `trinity-types` | **Full** | `SecretBytes` is the zeroize wrapper for passphrase and recovery words (`secret.rs`); `Balance::spend_policy_reference_sats` is the daily-cap base. A `Clone`/Debug leak or a wrong reference leaves key material or silently raises the cap. |
| `trinity-entropy` | **Full** | This crate produces the seed (`extract` = HMAC-SHA512 of CSPRNG and optional extra, then BIP-39). A silent degrade is the Coldcard 2026 class (2.1): a permanently weak seed, not repairable by update. |
| `trinity-keystore` | **Full** | Encrypted blobs, AEAD, slot policy, KEK unwrap via the platform callback. A wrong unwrap or a skipped wipe yields a signing key — and with the other on-device key, the quorum. |
| `trinity-signer` | **Full** | Adds the signatures and enforces `SpendPolicy` on `sign_ab` before `unwrap_kek`. A skipped verify-before-sign or a broken window spends funds the user did not authorize, or more than the daily cap. |
| `trinity-verify` | **Full** | Independent last check (V1–V10, 1.5) before any key access. A missed V-check signs a tampered PSBT — wrong recipient, inflated fee, substituted input. |
| `trinity-ffi` | **Full** | The only crate that will carry `#[uniffi::export]` (1.3). Today a WP-00 scaffold; the role is already specified. A leaked type across this boundary lets the JS layer touch key material — the trust boundary is gone. |
| `trinity-chain` | **Full** | `ChainBackend::broadcast` puts the transaction on the network; `full_scan`/`sync` produce the `Update` that becomes the displayed balance. A live broadcast reported as failure, or a success mapped to a network error, causes a second send or hides a spend. |
| `trinity-watch` | **Weaker** | Builds the descriptors and the PSBT that will be signed (`WatchWallet::build_psbt`). Per-send construction is independently re-checked by `trinity-verify` (V1–V10: declared recipient, amount, fee, scripts, known UTXOs) before any signature — that is why this crate is not full-tier. Residual: a grammatically valid but wrong descriptor persisted at setup becomes the wallet that verify will then enforce. Not absent, because that setup path is still money. |
| `trinity-transport` | **Deferred** | WP-00 scaffold (crate docs + `#![forbid(unsafe_code)]` only). Will carry PSBTs to hardware signers. Classification is due as soon as the crate has domain code. |
| `trinity-export` | **Deferred** | Same scaffold. Will emit recovery descriptors (Sparrow / BSMS / Core). A wrong export would recover into the wrong wallet; there is no such code yet. Classification is due as soon as the crate has domain code. |

**Audit (O11, 5.5 criterion 13):** the full-tier crates plus both platform keystore implementations. That scope is larger than the earlier four-crate list (`keystore`, `signer`, `verify`, `ffi`) and costs more; an audit that omits seed entropy or the path money leaves the device does not audit what the audit is for. Weaker-tier and deferred crates are out of that audit for the reasons in the table.

**Gates that are narrower, on purpose.** A silent second list is not allowed; a named subset is.

| Gate | Covers | Why narrower than this table |
|---|---|---|
| Dependency budget (1.7, `scripts/dep_budget.py`) | `trinity-types`, `-entropy`, `-keystore`, `-signer`, `-verify` | The five that see key material or independently verify a PSBT. Folding in `trinity-chain`, `-watch`, or `-ffi` would pull Electrum/Core/CBF, BDK-wallet, and uniffi into the ceiling and make it stop constraining the secret-handling path. |
| 100 % coverage (`docs/TESTING.md` §3.2) | the same five | `trinity-chain` keeps ≥ 90/85 % because the remainder is network error paths per backend; `trinity-ffi` keeps ≥ 95/90 % (scaffold + uniffi generator). Those thresholds are the gate, not a claim that the crates are off this surface. |
| Mutation testing (CI `cargo mutants`, `just mutants`) | `trinity-verify`, `-signer`, `-keystore`, `-entropy` | Omits `trinity-types` (value types; `SecretBytes` invariants are type-level: no `Clone`, redacted Debug — trybuild, not mutants). Omits `trinity-chain` and `trinity-ffi` (network backends / scaffold). Widening the CI invocation is a configuration change and is not made from this document. |
...
| App uninstall | ✅ **Resolved 2026-08-11 (Appendix B.4).** iOS Keychain items with `…ThisDeviceOnly` **do survive an uninstall** — confirmed mechanism (Apple's own doc for the constant only documents the backup/restore and passcode-removal deletion paths, not uninstall; an Apple DTS engineer confirmed on the record that persistence-through-uninstall is "an artefact of the implementation" and not a documented guarantee; independently reconfirmed as of a 2025 `expo-secure-store` issue). `blob_A`/`blob_B` disappear with the sandbox on uninstall, so the wrapped blobs are gone — but the **KEK-wrapping keys in the Keychain itself persist** and would silently apply to whatever a reinstall writes next. **Mandatory mitigation, not optional:** on first launch, check a `UserDefaults` marker (UserDefaults **is** cleared on uninstall, unlike Keychain); if absent, treat this as a fresh install and force-delete all app Keychain items before any provisioning. Implementing WP: **WP-41**. Not independently confirmed for iOS 19 specifically (most recent version-specific corroboration found: iOS 17.5) — re-verify on release-target OS during WP-41. |
...
| `nLockTime` | `= current tip height`, `nSequence = 0xFFFFFFFD` | Anti-fee-sniping: a reorg miner cannot pull the transaction into an older block (any value `< 0xFFFFFFFF` enforces `nLockTime`). Standard behavior of Core and Sparrow — deviation would be a fingerprint. |
| RBF | `nSequence = 0xFFFFFFFD` also signals BIP-125 replaceability | ✅ **Corrected 2026-08-12** (was inconsistently `0xFFFFFFFE` in this row, which enforces `nLockTime` but explicitly does **not** signal replaceability — that value would have silently broken every RBF path this document already specifies: S12, S29h, the fee-bump flow. `0xFFFFFFFD` is BDK's own default (`Sequence::ENABLE_RBF_NO_LOCKTIME`) and satisfies both properties at once — no separate mechanism needed. Fee increase without new key access possible; fee-bump flow runs the same verification. |
...
**Window time source — enforcement, not display.** The sliding window is **not** measured against wall-clock time. It may advance only by as much as is **provably** elapsed. Candidate sources: a monotonic, boot-relative clock with a persisted anchor; and the **block height** of the chain attachment (about **144 blocks per 24 hours as an expectation**, not a guarantee — difficulty and hash rate move the real interval). **Decided (WP-34, O18 option (c)):** both sources trustworthy → `min`; only the monotonic clock (same boot, offline) → that clock; monotonic clock not trustworthy (reboot) → no progress, even if a server claims a higher tip.
...
| **T5a** | **Snatched, unlocked phone** without knowledge of the passphrase — the most common real attack on phone wallets | A and B, limited | ⚠️ **Partial, and exactly here sits the main gain over single-sig.** The thief can spend up to the `SpendPolicy` limit (default `clamp(20 % of balance, €200, €500)` in 24 h — practically €200 at small and €500 at large holdings). Above that the **Rust core** requires the passphrase — `sign_ab` fails with `SpendLimitExceeded` and `sign_ab_with_passphrase` is the only path that unlocks B. Disabling the policy also only works with passphrase (3.6.4). **Depends on a tamper-resistant window clock (3.6.7, T21)** — without it the share can be drained repeatedly by setting the device clock. | ⚠️ **Loss up to the share.** The rest is recoverable: backup-B plus C into a fresh setup, with exactly the two keys the thief does not have. **On a single-sig wallet the same incident is a total loss without course of action.** Share user-settable to "always ask" (3.6.5). |
...
| **Dependency gates** | `cargo-deny`, `cargo-audit`, `cargo-vet`; dependency count of the §1.8 secret-handling subset ≤ 55, measured (1.7). |
...
| 9 | `cargo-deny`, `cargo-audit`, `cargo-vet` without open findings; dependency budget of the §1.8 secret-handling subset (`scripts/dep_budget.py`: `trinity-types`, `-entropy`, `-keystore`, `-signer`, `-verify`) within limit 55; measured **52 external crates** as the union over shipped targets `aarch64-apple-ios` and `aarch64-linux-android`. |
...
| 13 | External security audit of the security-critical surface (§1.8, full-tier: `trinity-types`, `-entropy`, `-keystore`, `-signer`, `-verify`, `-ffi`, `-chain`) plus both platform keystore implementations, for v1.0. That scope is larger than the earlier four-crate list and costs more; an audit that omits seed entropy or the path money leaves the device does not audit what the audit is for. Weaker-tier `trinity-watch` and deferred `trinity-transport`/`trinity-export` are out of this audit (reasons in §1.8). Critical and high severity findings closed. |
...
| ~~**O3**~~ | ~~Default chain backend~~ | — | — | ✅ **Decided 2026-08-11, gap 3 now closed.** **(a) CBF as default**, honest label "more private than a third-party server, not anonymous" (block-fetch goes to a uniformly random peer per the resolved gap-3 finding — not a hardened anti-correlation design). Implementing WP: **WP-16**. |
...
| **O11** | Timing of the external security audit | (a) before v1.0 · (b) after v1.0 with limited beta circle · (c) none | An audit before v1.0 delays; one after exposes real money to an unaudited security-critical surface (§1.8). | **(a) before v1.0**, scope: the **full-tier** crates of §1.8 (`trinity-types`, `-entropy`, `-keystore`, `-signer`, `-verify`, `-ffi`, `-chain`) and both platform keystore implementations. That is larger than the earlier four-crate list, and it costs more; omitting entropy or the broadcast/balance path would not audit the failure classes this product is built to avoid (2.1, 1.6). Weaker-tier `trinity-watch` and deferred `trinity-transport`/`trinity-export` stay out of this audit (reasons in §1.8). Critical and high findings are release blockers (5.5, criterion 13). |
...
| ~~**O18**~~ | ~~Time source of the spend window~~ | — | — | ✅ **Decided (WP-34, 2026-08-14): Option (c)** — both trustworthy → `min`; only the monotonic clock (same boot, offline) → that clock; monotonic not trustworthy (reboot) → no progress (block height is a `min` brake, never a standalone source). Wall clock is display / fail-closed veto only. Implemented in 3.6.7. |
...
**Status (2026-08-11):** **9 of 14** points closed (B.1, B.2, B.3, B.4, B.5, B.6, B.7, B.8, B.9); **5 still open** (B.10–B.14). All five remaining are v1.1-scope hardware-signer device behaviors (NFC/BLE specifics, firmware version reporting) — deliberately deferred per E6 ("BLE for BitBox02 Nova and Ledger in v1.1"); they block v1.1 hardware work, not the v1 core build now underway.
...
| ~~**4**~~ | ~~Do Keychain items with `…ThisDeviceOnly` survive an app uninstall under iOS 17/18/19?~~ | 2.6 | ✅ **Resolved 2026-08-11.** Yes, they survive uninstall (Apple doc + DTS confirmation + 2025 `expo-secure-store` reconfirmation — not officially guaranteed, but consistently observed since 2015). Mandatory `UserDefaults`-marker wipe-on-first-launch mitigation recorded in 2.6. Not iOS-19-specific — re-verify during WP-41. |
